...
int main(int argc, char** argv) {
    ...
    if(argc != 5)
	{
		...
		cout << "./main <problem-flag> <SARS-COV2 genome file> <number_of_random_segment> <mutation rate>" << endl;
		cout << "Example:" << endl;
		cout << "./main problem1A SARS_COV2_genome_file.txt 1000 5"  << endl;
		cout << "=========================="<< endl << endl;
		cout << "exiting..." << endl;
		exit(-1);
	}
    ...
    char **segments = new char*[strlen(genome)-36+1];
    for(int i = 0; i < strlen(genome)-36+1; i++) {
        segments[i] = new char[37];
        strncpy(segments[i], &genome[i], 36);
        segments[i][36] = '\0';
    }

    if(strcmp(argv[1], "problem1A") == 0) {

        char** res = random_36_mers(genome, atoi(argv[3]), 36, 0);
        prefix_trie *obj = new prefix_trie(res, atoi(argv[3]), 36);

        int x = 0;
        for(int i = 0; i < strlen(genome)-36+1; i ++) {
            //cout << res[i] << endl;
            bool temp = obj->search(segments[i], 1);
            if(temp == true) {
                x++;
            }
            delete[] segments[i];
        ...
        cout << "found " << x << " in the sequence"<< endl;
        cout << "Total Nodes: " << obj->get_node_num() << endl;
        delete[] res;
        delete[] segments;
    }
    ...
    if(strcmp(argv[1], "problem1B") == 0) {
        ...
        char** res = random_36_mers(genome, atoi(argv[3]), 36, atoi(argv[4]));
        prefix_trie *obj = new prefix_trie(res, atoi(argv[3]), 36);
...
char** random_36_mers(char *genome, int number_of_sequence, int sequence_length, int mutation_percent) {
    ...
    if(mutation_percent != 0) {
        ...
        for(int i = 0; i < len-36; i ++) {
            ...
            int temp = rand() % len;
            if((temp/float(len))*100 <= mutation_percent ) {        //5% chance to change the base
                int ran = rand() % 3;
                if(genome[i] == 'A')
                {
                    genome[i] = temp1[ran];
